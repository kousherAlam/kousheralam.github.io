...
export default function Certificate(data: CertificateProp) {
  return (
    <div className="shadow bg-slate-50 w-full">
      <div className="grid grid-cols-12">
        <div className="flex justify-center items-center col-span-3 bg-slate-100 py-2 px-1">
          <img src={data.badge} alt="Google Verification badge" />
        </div>
        <div className="col-span-9 px-3 py-4 flex flex-col justify-between">
          <div>
            <h4 className="text-lg text-black">{data.title}</h4>
          ...
          <div>
            <a
              className="text-sm text-orange-600 hover:text-orange-500 visited:text-orange-800"
              href={data.verify}
              target="_blank"
            >
              Verify Certificate
